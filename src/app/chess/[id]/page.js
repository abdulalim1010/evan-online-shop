...
export default async function ChessDetailsPage({ params }) {
  const { id } = await params;
  const item = await getChess(id);
  ...
  return (
    <div className="page-shell fade-up">
      <Link href="/chess" className="text-blue-600 hover:underline mb-4 inline-block">
        ← Back to Home
      </Link>

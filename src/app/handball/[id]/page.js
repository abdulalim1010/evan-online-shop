...
export async function generateMetadata({ params }) {
  ...
  const item = await getHandball(id);
  return {
...
export default async function HandballDetailsPage({ params }) {
  const { id } = await params;
  const item = await getHandball(id);
  ...
  return (
    <div className="page-shell fade-up">
      <Link href="/handball" className="text-blue-600 hover:underline mb-4 inline-block">
        ← Back to Home
      </Link>

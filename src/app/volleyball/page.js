...
export default function VolleyballPage() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch("/api/volleyball");
  ...
  return (
    <div className="page-shell fade-up">
      <div className="mb-8">
        <h1 className="text-3xl font-bold md:text-4xl">Volleyball Collection</h1>
        <p className="mt-2 text-slate-600">Indoor and beach volleyballs, knee pads and accessories.</p>
      </div>
      
      {products.length === 0 ? (
        <div className="glass-card text-center py-12">
          <p className="text-slate-500 text-lg">No volleyball products available yet.</p>

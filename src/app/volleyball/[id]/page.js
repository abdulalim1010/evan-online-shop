...
async function getVolleyball(id) {
  try {
...
export async function generateMetadata({ params }) {
  ...
  const item = await getVolleyball(id);
  return {
...
export default async function VolleyballDetailsPage({ params }) {
  const { id } = await params;
  const item = await getVolleyball(id);
  ...
  return (
    <div className="page-shell fade-up">
      <Link href="/volleyball" className="text-blue-600 hover:underline mb-4 inline-block">
        ← Back to Home
      </Link>
